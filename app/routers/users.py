...
def create_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    if user.email is None or user.email.strip() == "" or user.hashed_password is None or user.hashed_password.strip() == "" or user.first_name is None or user.first_name.strip() == "" or user.last_name is None or user.last_name.strip() == "" or user.account_type is None or user.account_type.strip() == "" or user.bussiness_type is None or user.bussiness_type.strip() == "":
    ...
    if user.account_type != "INDIVIDUAL" and user.account_type != "COMPANY":
        raise HTTPException(status_code=400, detail="Invalid account type")
    hashed_password = user.hashed_password.strip()
    if hashed_password == "" or hashed_password == "string" or hashed_password.__contains__(" "):
...
def update_user_password(user: schemas.UserPassword, db: Session = Depends(get_db), current_user: schemas.User = Depends(get_current_active_user)):
    ...
    if db_user is None:
        raise HTTPException(status_code=404, detail="User not found")
    user.hashed_password = user.hashed_password.strip()
    if user.hashed_password == "" or user.hashed_password == "string" or user.hashed_password.__contains__(" "):
        raise HTTPException(status_code=400, detail="Invalid password")
