...
def create_role(role: schemas.RoleCreate, db: Session = Depends(get_db), basic_auth=Depends(basic_auth)):
    try:
...
def read_roles(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), basic_auth=Depends(basic_auth)):
    try:
...
def read_role(role_name: str, db: Session = Depends(get_db), basic_auth=Depends(basic_auth)):
...
def update_role(role: schemas.RoleUpdate, db: Session = Depends(get_db), basic_auth=Depends(basic_auth)):
...
def delete_role(role_name: str, db: Session = Depends(get_db), basic_auth=Depends(basic_auth)):
