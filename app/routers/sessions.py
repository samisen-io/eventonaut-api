...
@router.get("/sessions/{uuid}", response_model=list[schemas.Session])
def get_sessions_by_conference_id(uuid: str, db: Session = Depends(get_db)):
    if conferences_crud.get_conference_by_uuid(db, uuid=uuid) is None:
        raise HTTPException(status_code=404, detail="Conference not found")
    db_sessions = crud.get_all_sessions_by_uuid_id(db, conference_id=uuid)
    if db_sessions is None or len(db_sessions) == 0:
        raise HTTPException(status_code=404, detail="Session not found")
    return db_sessions
...
def delete_session(uuid: str, db: Session = Depends(get_db), current_user: uschemas.User = Depends(get_current_active_user)):
    db_session = crud.get_session_by_uuid_id(db, uuid=uuid, owner_id=current_user.id)
    if db_session is None:
        raise HTTPException(status_code=404, detail="Session not found")
    return crud.delete_session(db=db, uuid=uuid, owner_id=current_user.id)