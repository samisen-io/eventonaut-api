...
from app.oauth2 import get_current_active_user, get_current_active_organization
...
def create_settings(settings:schemas.SettingsCreate, db: Session = Depends(get_db), current_organization: OrganizationSecurity = Security(get_current_active_organization, scopes=[RoleEnum.ORGANIZATION_ADMIN.name, RoleEnum.ORGANIZATION_USER.name, "organizer"])):
    if conferences_crud.get_conference_by_uuid(db, uuid=settings.conference_id,organization_id=current_organization.id) is None:
        logging.exception("Conference not found")
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Conference not found")
    db_settings = crud.get_settings_by_conference_uuid(db, conference_uuid=settings.conference_id,organization_id=current_organization.id)
    if db_settings:
...
def update_settings(settings: schemas.SettingsCreate,db: Session = Depends(get_db), current_organization: OrganizationSecurity = Security(get_current_active_organization, scopes=[RoleEnum.ORGANIZATION_ADMIN.name, RoleEnum.ORGANIZATION_USER.name, "organizer"])):
    if conferences_crud.get_conference_by_uuid(db, uuid=settings.conference_id,organization_id=current_organization.id) is None:
        logging.exception("Conference not found")
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Conference not found")
    db_settings = crud.get_settings_by_conference_uuid(db, conference_uuid=settings.conference_id,organization_id=current_organization.id)
...
def delete_settings(conference_id: str, db: Session = Depends(get_db), current_organization: OrganizationSecurity = Security(get_current_active_organization, scopes=[RoleEnum.ORGANIZATION_ADMIN.name, RoleEnum.ORGANIZATION_USER.name, "organizer"])):
    db_settings = crud.get_settings_by_conference_uuid(db, conference_uuid=conference_id,organization_id=current_organization.id)
    if db_settings is None:
        logging.exception("Settings not found")
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Settings not found")
