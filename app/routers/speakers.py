...
def get_speakers(offset: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def update_speaker(speaker: schemas.SpeakerUpdate, db: Session = Depends(get_db)):
    ...
    if speaker.conference_id is not None:
        if conferences_crud.get_conf_by_uuid(db, speaker.conference_id) is None:
