...
def get_all_venues_by_owner_id(db: Session, owner_id: int, offset: int, limit: int):
    return db.query(models.Venue).filter(models.Venue.owner_id == owner_id, models.Venue.isarchived == False).offset(offset).limit(limit).all()

def get_venue_by_id(db: Session, venue_id: str, owner_id: int):
    return db.query(models.Venue).filter(models.Venue.uuid == venue_id, models.Venue.owner_id == owner_id, models.Venue.isarchived == False).first()
