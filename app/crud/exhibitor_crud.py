...
def get_exhibitors(db: Session, conference_id: int):
    return db.query(Exhibitor).filter(Exhibitor.conference_id == conference_id, Exhibitor.is_archived == False).all()

def get_exhibitor_by_id(db: Session, exhibitor_id: str):
    return db.query(Exhibitor).filter(Exhibitor.uuid == exhibitor_id).filter(Exhibitor.is_archived == False).first()

def create_exhibitor(db: Session, exhibitor: schemas.ExhibitorCreate, conference_id: int):
    ...
    db_exhibitor.conference_id = conference_id
...
def update_exhibitor(db: Session, db_exhibitor: Exhibitor, exhibitor: schemas.ExhibitorUpdate, conference_id: int):
    exhibitor_dict = exhibitor.model_dump()
    exhibitor_dict.pop('id', None)
    exhibitor_logo = exhibitor_dict.pop('exhibitor_logo', None)
    exhibitor_banner = exhibitor_dict.pop('exhibitor_banner', None)
    exhibitor_dict['conference_id'] = conference_id
