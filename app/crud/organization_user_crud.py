...
def get_users_by_organization_uuid(db: Session, organization_uuid: str):
    ...
    if organization is None:
        raise HTTPException(status_code=404, detail="Organization with the specified ID not found")
    ...
    for organization_user in organization.organization_user:
        ...
        users.append(schemas.OrganizationUserBase(
            ...
            user=schemas.UserBase(uuid=user.uuid, email= user.email or '')
        ))
...
def get_organizations_by_user_uuid(db: Session, user_uuid: str):
    ...
    if user is None:
        raise HTTPException(status_code=404, detail="User with the specified ID not found")
