...
def get_current_user_RT(db: Session = Depends(get_db), data: str =  Depends(oauth_2_scheme)):
    # def get_current_user_RT(data: str, db: Session = Depends(get_db) Depends(oauth_2_scheme)):
    credentials_exception = HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
                                          detail="Could not validate credentials")
